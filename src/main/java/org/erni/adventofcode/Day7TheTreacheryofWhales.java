...
public class Day7TheTreacheryofWhales {
    ...
    public static int aligningCost(int position, Map<Integer, MutableInt> map) {
        int cost = 0;
        for (int pos : map.keySet()) {
            cost += map.get(pos).get() * Math.abs(position - pos);
        }
        return cost;
    }

    public static void part1() {
        Map<Integer, MutableInt> positionsMap = new HashMap<>();
        int max = 0;

        for (int pos : positions) {
            if (pos > max) max = pos;

            MutableInt count = positionsMap.get(pos);
            if (count == null) {
                MutableInt init = new MutableInt();
                positionsMap.put(pos, init);
            }
            else {
                count.increment();
            }
        ...
        int lessCost = aligningCost(0, positionsMap);
        for (int i = 1; i <= max; i++) {
            int currentCost = aligningCost(i, positionsMap);
            if (currentCost < lessCost) lessCost = currentCost;
        }

        System.out.println("Less cost is: " + lessCost);
    }

    public static int aligningCost2(int position, Map<Integer, MutableInt> map) {
        int cost = 0;
        for (int pos : map.keySet()) {
            if (pos == position) continue;
            int steps = Math.abs(position - pos);
            float oneCrabCost = (steps + 1) * ((float) steps/2);
            cost += map.get(pos).get() * oneCrabCost;
        }
        return cost;
    }
    ...
    public static void part2() {
        ...
        for (int pos : positions) {
            ...
            MutableInt count = positionsMap.get(pos);
            if (count == null) {
                positionsMap.put(pos, new MutableInt());
            }
            else {
                count.increment();
            }
        ...
        long leastCost = aligningCost2(486, positionsMap);
        int leastCostPos = 0;
        /*for (int i = 1; i <= max; i++) {
            long currentCost = aligningCost2(i, positionsMap);
        ...
        System.out.println("Least cost position is: " + leastCostPos);
    }

    public static long calculateLeastFuelConsumption(
            List<Integer> horizontalPositions,
            BiFunction<Long, Integer, Long> transformer
    ) {
        return LongStream
                .rangeClosed(
                        horizontalPositions.stream().min(Integer::compareTo).orElse(Integer.MIN_VALUE),
                        horizontalPositions.stream().max(Integer::compareTo).orElse(Integer.MIN_VALUE)
                )
                .map(i -> horizontalPositions.stream().map(p -> transformer.apply(i, p)).reduce(Long::sum).orElse(Long.MAX_VALUE))
                .min()
                .orElseGet(() -> Long.MIN_VALUE);
    }

    public static BiFunction<Long, Integer, Long> facultyDistance() {
        return (a, b) -> LongStream.rangeClosed(1, Math.abs(a - b)).reduce(0, Long::sum);
    }


    public static void part2B() {
        int[] crabPositions = positions;
        final int MIN_POSITION, MAX_POSITION;
        MIN_POSITION = Arrays.stream(crabPositions).min().getAsInt();
        MAX_POSITION = Arrays.stream(crabPositions).max().getAsInt();

        int minFuel = Integer.MAX_VALUE;
        int leastCostPosition = 0;
        for (int pos = MIN_POSITION; pos <= MAX_POSITION; pos++) {
            int fuel = 0;
            for (int crabPosition : crabPositions) {
                int diff = Math.abs(crabPosition - pos);
                fuel += (diff * (diff + 1)) / 2;
            }
            if (fuel < minFuel) {
                minFuel = fuel;
                leastCostPosition = pos;
            }
        }
        System.out.println("Part 2 B: " + minFuel);
        System.out.println("Part 2 B - position: " + leastCostPosition);
    }

    public static void checkPosition486() {
        int cost = 0;
        for (int pos : positions) {
            int currentDistance = Math.abs(486 - pos);
            int oneCrabCost = 0;
        ...
        System.out.println("Position 486 cost is: " + cost);
    }

    public static void checkPosition486_B() {
        int costA = 0;
        int costB = 0;
        int previousCostA = 0;
        int previousCostB = 0;
        ...
        for (int pos : positions) {
            int currentDistance = Math.abs(486 - pos);
            float oneCrabCostFloat = (currentDistance + 1) * ((float) currentDistance/2);
            int oneCrabCostInt = 0;
            for (int i = 1; i <= currentDistance; i++) {
                oneCrabCostInt += i;
            }

            if (oneCrabCostInt != oneCrabCostFloat) System.out.println("INT and FLOAT values ARE DIFFERENT!!!");

            previousCostA = costA;
            previousCostB = costB;
            costA += oneCrabCostInt;
            costB += oneCrabCostFloat;

            if (costA != costB) {
                System.out.println("INT COST IS DIFFERENT THAN FLOAT COST");
            }
        }
        System.out.println("Position 486 cost B is: " + costB);
    }

    public static void checkCountsInMap() {
        Map<Integer, MutableInt> positionsMap = new HashMap<>();
        int max = 0;
        ...
        for (int pos : positions) {
            MutableInt currentNumInPos = positionsMap.get(pos);
            int currentPostCount = 0;
            if (currentNumInPos == null) System.out.println("A POSITION IS NULL!!!!");
            else {
                for (int i = 0; i < positions.length; i++) {
                    if (positions[i] == pos) currentPostCount++;
                }
                if (currentPostCount != currentNumInPos.get()) System.out.println("THEY´RE DIFFERENT!!!");
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        Collections.addAll(list,positionsInteger);
        System.out.println(calculateLeastFuelConsumption(list, facultyDistance())); // right one 98039527
         */
    }
